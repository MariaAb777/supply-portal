    ...
    <AppTableCard>
      ...
      >
        ...
        <template #body-cell-kpId="props">
          <q-td :props="props">
            <router-link v-if="props.row.id === '1'" :to="{ name: 'kp-card', params: { id: props.row.kpId } }" class="text-accent text-weight-bold">
              {{ props.row.kpId }}
            </router-link>
            <span v-else class="text-weight-bold text-lighttext">{{ props.row.kpId }}</span>
        ...
        <template #body-cell-status="props">
          <q-td :props="props">
            <StatusBadge :label="props.row.status" :tone="props.row.statusTone" />
          </q-td>
        </template>
      </q-table>
      <template #footer>
        <AppPagination v-model="pagination" :total-rows="filteredRows.length" />
      </template>
    </AppTableCard>
  </div>
</template>
...
const statusOptions = homeStageChips.map((chip) => ({
  label: chip.id === "all" ? "Все статусы" : chip.label,
  value: chip.id
}));
...
const filteredRows = computed(() => {
  ...
  return kpRows.filter((row) => {
    ...
    const matchesStatus = statusFilter.value === "all" || row.stage === statusFilter.value;
...
watch([search, statusFilter, quantityFilter], () => {
